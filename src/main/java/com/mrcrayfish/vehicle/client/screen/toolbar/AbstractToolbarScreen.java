...
public abstract class AbstractToolbarScreen extends Screen
{
    ...
    protected void init()
    {
        ...
        {
            if (widgets.get(i) instanceof List<?> list && !list.isEmpty() && list.get(0) instanceof Button) {
                for (Object obj : list) {
                    Button widget = (Button) obj;
        ...
        for(int i = 0; i < widgets.size(); i++)
        {
            Button widget = (Button) widgets.get(i);
    ...
    {
        matrixStack.fillGradient(0, this.height / 2, this.width, this.height, 0x00000000, 0xAA000000);
        ...
        {
            if(widget instanceof Button bt)
            {
                if(CommonUtils.isMouseWithin(mouseX, mouseY, bt.x, bt.y, bt.getWidth(), bt.getHeight()))
                {
                    hoveredWidget = bt;
                    break;
                }
        ...
        if(hoveredWidget instanceof IToolbarLabel)
        {
            ...
            matrixStack.drawString(this.minecraft.font, message, this.width / 2 - messageWidth / 2, startY - 12, 0xFFFFFF);
        }
    }
