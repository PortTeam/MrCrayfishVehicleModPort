...
public class DoorButton extends IconButton
{
    ...
    public DoorButton(VehicleEntity entity, CosmeticProperties properties, OpenableAction action)
    {
        super(20, 20, ICON_MAP.getOrDefault(properties.getId(), DashboardScreen.Icons.LEFT_DOOR), Component.translatable(properties.getId().getNamespace() + ".toolbar.label." + properties.getId().getPath()), onPress -> {
            PacketHandler.getChannel().sendToServer(new MessageInteractCosmetic(entity.getId(), properties.getId()));
        });
        this.action = action;
    }
    ...
    public void renderWidget(GuiGraphics matrixStack, int mouseX, int mouseY, float partialTicks)
    {
        ...
        matrixStack.fill(this.x + 3, this.y + this.height - 6, this.x + this.width - 3, this.y + this.height - 3, backgroundColor);
        matrixStack.fill(this.x + 3, this.y + this.height - 6, this.x + this.width - 4, this.y + this.height - 4, foregroundColor);
    }
