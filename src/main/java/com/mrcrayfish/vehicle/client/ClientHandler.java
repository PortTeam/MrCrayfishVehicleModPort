...
//        event.registerBlockEntityRenderer(ModTileEntities.GAS_PUMP_TANK.get(), GasPumpTankRenderer::new);
    ...
    private static void setupItemColors()
    {
        ...
        // Define the color provider
//        ItemColors colorProvider = (index, stack) -> {
...
//        };
        ...
        // Register colors for all items
        for (Item item : ForgeRegistries.ITEMS) {
            if (item instanceof DyeItem || (item instanceof PartItem && ((PartItem) item).isColored())) {
                //itemColors.register((ItemColor) colorProvider, item);
            }
        }
    }
