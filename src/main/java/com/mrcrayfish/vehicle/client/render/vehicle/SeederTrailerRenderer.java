...
public class SeederTrailerRenderer extends AbstractTrailerRenderer<SeederTrailerEntity>
{
                    ...
                    matrixStack.pushPose();
                    {
                        ...
                        for(int j = 0; j < count; j++)
                        {
                            matrixStack.pushPose();
                            {
                                ...
                                Minecraft.getInstance().getItemRenderer().renderStatic(stack, ItemDisplayContext.NONE, light, OverlayTexture.NO_OVERLAY, matrixStack, renderTypeBuffer,stack.getFrame().level(), 3);
                            }
                            matrixStack.popPose();
                            index++;
    ...
    private void renderSpike(SeederTrailerEntity vehicle, PoseStack matrixStack, MultiBufferSource renderTypeBuffer, double offsetX, float partialTicks, int light)
    {
        ...
        matrixStack.mulPose(com.mojang.math.Axis.XP.rotationDegrees(-this.getWheelRotation(vehicle, null, partialTicks)));
