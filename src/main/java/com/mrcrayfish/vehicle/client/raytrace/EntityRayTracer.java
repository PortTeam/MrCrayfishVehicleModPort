...
public class EntityRayTracer
{
     ...
     */
    private <T extends VehicleEntity> void generateEntityTriangles(EntityType<T> entityType, Map<RayTraceData, List<Matrix4f>> transforms)
    {
    ...
    public static float[] getTransformedTriangle(float[] triangle, Matrix4f matrix)
    {
        ...
        for (int i = 0; i < 9; i += 3)
        {
            Quaternionf vec = new Quaternionf(triangle[i], triangle[i + 1], triangle[i + 2], 1);
            Vector4f vec4 = new Vector4f(triangle[i], triangle[i + 1], triangle[i + 2],1);
            vec.transform(vec4);
            triangleNew[i] = vec.x();
    ...
    private <T extends VehicleEntity> VehicleRayTraceResult rayTraceEntities(boolean rightClick)
    {
        ...
        for(VehicleEntity entity : Objects.requireNonNull(minecraft.level).getEntitiesOfClass(VehicleEntity.class, box))
        {
            ...
            {
                VehicleMod.LOGGER.warn("The vehicle '" + type.toString() + "' does not have any registered ray trace transforms.");
            }
        }
    ...
    private <T extends VehicleEntity> void initializeTransforms(EntityType<T> type)
    {
        if(!this.entityRayTraceData.containsKey(type))
        {
            ...
            HashMap<RayTraceData, List<Matrix4f>> parts = Maps.newHashMap();
    ...
    private <T extends VehicleEntity> void generateInteractableBoxes(EntityType<T> type, List<Matrix4f> transforms)
    {
    ...
    private boolean processHit(VehicleRayTraceResult result)
    {
        ...
        if(!mc.player.isCrouching() && entity instanceof VehicleEntity && data instanceof CosmeticRayTraceData)
        {
            ...
            if(!actions.isEmpty())
            {
                ...
                PacketHandler.getChannel().sendToServer(new MessageInteractCosmetic(vehicle.getId(), cosmeticId));
                return true;
            }
        }
        ...
        {
            PacketHandler.getChannel().sendToServer(new MessageInteractKey(entity));
            return true;
        }
        ...
        if(isContinuous || !(mc.hitResult != null && mc.hitResult.getType() == HitResult.Type.ENTITY && ((EntityHitResult) mc.hitResult).getEntity() == entity))
        {
            ...
            if(notRiding)
            {
                ...
                {
                    PacketHandler.getChannel().sendToServer(new MessagePickupVehicle(entity));
                    return true;
                }
                if(!isContinuous)
                {
