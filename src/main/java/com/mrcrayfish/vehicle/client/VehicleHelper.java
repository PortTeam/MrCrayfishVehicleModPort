...
public class VehicleHelper
{
    ...
    public static void playSound(SoundEvent soundEvent, BlockPos pos,RandomSource randomSource, float volume, float pitch)
    {
        SimpleSoundInstance sound = new SimpleSoundInstance(soundEvent, SoundSource.BLOCKS, volume, pitch,randomSource ,pos.getX() + 0.5F, pos.getY(), pos.getZ() + 0.5F);
    ...
    public static float getThrottle(LivingEntity livingEntity)
    {
        if(ClientHandler.isControllableLoaded())
        {
            ...
            if(controller != null && Controllable.getInput().isControllerInUse())
            {
                boolean forward = controller.isButtonPressed(ControllerHandler.ACCELERATE.getButton());
                boolean reverse = controller.isButtonPressed(ControllerHandler.REVERSE.getButton());
                if(forward && !reverse)
                {
    ...
    public static void spawnWheelParticle(BlockPos pos, BlockState state, double x, double y, double z, Vec3 motion)
    {
        ...
        {
            //TODO::ADD PARTICLES
//            Particle particle = new DiggingParticle(world, x, y, z, motion.x, motion.y, motion.z, state);
//            particle.init(pos);
