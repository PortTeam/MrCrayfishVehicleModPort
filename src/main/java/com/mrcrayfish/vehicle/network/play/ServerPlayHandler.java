...
public class ServerPlayHandler
{
    public static void handleAttachChestMessage(ServerPlayer player, MessageAttachChest message)
    {
        ...
        Entity targetEntity = Level.getEntity(message.getEntityId());
        if(targetEntity instanceof IAttachableChest)
        {
            float reachDistance = (float) player.getAttribute(ForgeMod.BLOCK_REACH.get()).getValue();
            if(player.distanceTo(targetEntity) < reachDistance)
            {
                ...
                if(!attachableChest.hasChest(message.getKey()))
                {
                    ...
                    if(!stack.isEmpty() && stack.getItem() == Items.CHEST)
                    {
                        ...
                        Level.playSound(null, targetEntity.getX(), targetEntity.getY(), targetEntity.getZ(), SoundType.WOOD.getPlaceSound(), SoundSource.BLOCKS, 1.0F, 1.0F);
                    }
                }
            }
        }
    ...
    public static void handleCraftVehicleMessage(ServerPlayer player, MessageCraftVehicle message)
    {
        ...
        WorkstationRecipe recipe = WorkstationRecipes.getRecipe(entityType, Level);
        ...
        Level.addFreshEntity(new ItemEntity(Level, message.getPos().getX() + 0.5, message.getPos().getY() + 1.125, message.getPos().getZ() + 0.5, stack));
    }
