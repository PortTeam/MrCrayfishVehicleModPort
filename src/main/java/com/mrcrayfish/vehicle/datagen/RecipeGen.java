...
//        CustomRecipeBuilder.special(ModRecipeSerializers.REFILL_SPRAY_CAN.get()).save(consumer, "vehicle:refill_spray_can");
    ...
    private static void netheriteSmithing(Consumer<FinishedRecipe> consumer, Item inputItem, Item resultItem)
    {
        ...
        SmithingTransformRecipeBuilder.smithing(Ingredient.of((ItemLike) Ingredient.EMPTY),Ingredient.of(inputItem), Ingredient.of(Items.NETHERITE_INGOT), RecipeCategory.MISC ,resultItem).unlocks("has_netherite_ingot", has(Items.NETHERITE_INGOT)).save(consumer, new ResourceLocation(id.getNamespace(), id.getPath() + "_smithing"));
    }
    ...
    {
        ResourceLocation entityId = ResourceLocation.tryParse(Objects.requireNonNull(type.toString()));
    ...
    private static void fluidExtracting(Consumer<FinishedRecipe> consumer, Item provider, FluidEntry output)
    {
        ResourceLocation id = ResourceLocation.tryParse(Objects.requireNonNull(output.getFluid().toString()));
    ...
    private static void fluidMixing(Consumer<FinishedRecipe> consumer, FluidEntry inputOne, FluidEntry inputTwo, Item provider, FluidEntry output)
    {
        ResourceLocation id = ResourceLocation.tryParse(Objects.requireNonNull(output.getFluid().toString()));
