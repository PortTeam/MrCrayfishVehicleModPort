 ...
 * Author: MrCrayfish
 */
public class JackTileEntity extends TileEntitySynced implements BlockEntityTicker<JackTileEntity>
{
    ...
    {
        super(ModTileEntities.JACK.get(),state,pos);
    }
    ...
    @Override
    public void tick(Level level, BlockPos blockPos, BlockState blockState, JackTileEntity jackTileEntity)
    {
        if(!this.activated && this.liftProgress == 0 && this.prevLiftProgress == 1)
        {
            this.level.setBlock(this.worldPosition, this.getBlockState().setValue(JackBlock.ENABLED, false), 3);
        }
        ...
        if(this.jack != null)
        {
            if(this.jack.getPassengers().size() > 0)
            {
                if(!this.activated)
                {
                    ...
                    this.activated = true;
                    this.level.setBlock(this.worldPosition, this.getBlockState().setValue(JackBlock.ENABLED, true),3);
                }
            }
