...
public class CosmeticTracker
{
    ...
    public void tick(VehicleEntity vehicle)
    {
        ...
        {
            PacketHandler.getChannel().send(PacketDistributor.TRACKING_ENTITY.with(() -> vehicle), new MessageSyncCosmetics(vehicle.getId(), this.getDirtyEntries()));
            this.resetDirty();
        }
        ...
        if(!vehicle.level.isClientSide())
        {
            if(!this.dirtyActions.isEmpty())
            {
                this.dirtyActions.forEach((cosmeticId, actions) ->
                {
                    ...
                    PacketHandler.getChannel().send(PacketDistributor.TRACKING_ENTITY.with(() -> vehicle), new MessageSyncActionData(vehicle.getId(), cosmeticId, actionData));
    ...
    {
        if(tag.contains("Cosmetics", CompoundTag.TAG_LIST))
        {
            ListTag list = tag.getList("Cosmetics", CompoundTag.TAG_COMPOUND);
            list.forEach(nbt -> {
