...
public class SeatTracker
{
    ...
    public void setSeatIndex(int index, UUID uuid)
    {
        ...
        {
            PacketHandler.getChannel().send(PacketDistributor.TRACKING_ENTITY.with(() -> vehicle), new MessageSyncPlayerSeat(vehicle.getId(), index, uuid));
        }
    }
    ...
    {
        if(compound.contains("PlayerSeatMap", CompoundTag.TAG_LIST))
        {
