...
public class VehicleTrailerEntity extends TrailerEntity
{
    ...
    protected void positionRider(Entity pPassenger, MoveFunction pCallback) {
        ...
        if(pPassenger instanceof VehicleEntity)
        {
            Vec3 offset = ((VehicleEntity) pPassenger).getProperties().getTrailerOffset().yRot((float) Math.toRadians(-this.yRot));
            pPassenger.setPos(this.getX() + offset.x, this.getY() + getPassengersRidingOffset() + offset.y, this.getZ() + offset.z);
            pPassenger.yRotO = this.yRotO;
            pPassenger.yRot = this.yRot;
        }
    }
