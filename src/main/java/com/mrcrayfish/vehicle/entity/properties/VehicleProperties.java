...
public class VehicleProperties
{
    ...
    public static class Serializer implements JsonDeserializer<VehicleProperties>, JsonSerializer<VehicleProperties>
    {
        ...
        private void readCamera(Builder builder, JsonObject object)
        {
        ...
        private void readExtended(Builder builder, JsonObject object)
        {
        ...
        private void readCosmetics(Builder builder, JsonObject object)
        {
    ...
    public static class Manager extends SimplePreparableReloadListener<Map<ResourceLocation, VehicleProperties>> {
        ...
        @Override
        protected Map<ResourceLocation, VehicleProperties> prepare(ResourceManager manager, ProfilerFiller profilerFiller) {
            ...
            manager.listResources(PROPERTIES_DIRECTORY, location -> location.getPath().endsWith(FILE_SUFFIX))
                            .forEach((location, resource) -> {
                                try {
                                    Optional<Resource> optionalStream = manager.getResource(location);
                                    if (optionalStream.isPresent()) {
                                        InputStream stream = optionalStream.get().open();
                                    ...
                                    VehicleMod.LOGGER.error("Couldn't parse vehicle properties {}", location, e);
                                }
                            });
            ...
            propertiesMap.forEach((id, properties) -> {
                if (properties.getCosmetics().isEmpty()) return;

                Map<ResourceLocation, List<Pair<ResourceLocation, List<ResourceLocation>>>> modelMap = new HashMap<>();
                manager.listResources(COSMETICS_DIRECTORY, fileName -> fileName.equals(id.getPath() + FILE_SUFFIX)).entrySet()
                        .stream()
                        .sorted(Comparator.comparing(entry -> {
                            String namespace = entry.getKey().getNamespace();
                            return namespace.equals(Reference.MOD_ID) ? 1 : 0;
                        }))
                        .forEach(entry -> {
                            ResourceLocation location = entry.getKey();
                            ResourceLocation vehicleId = format(location, COSMETICS_DIRECTORY);
                            if (!vehicleId.getNamespace().equals(id.getNamespace())) return;
        ...
        public void addReloadListenerEvent(AddReloadListenerEvent event) {
            event.addListener(Manager.instance = new Manager());
        }
