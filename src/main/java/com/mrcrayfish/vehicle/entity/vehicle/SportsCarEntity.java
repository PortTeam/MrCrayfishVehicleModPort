...
public class SportsCarEntity extends LandVehicleEntity implements IStorage
{
    ...
    public static void registerInteractionBoxes()
    {
        EntityRayTracer.instance().registerInteractionBox(ModEntities.SPORTS_CAR.get(), () -> {
            ...
            if(rightClick) {
                PacketHandler.getChannel().sendToServer(new MessageOpenStorage(entity.getId(), GLOVE_BOX_STORAGE_KEY));
        ...
        EntityRayTracer.instance().registerInteractionBox(ModEntities.SPORTS_CAR.get(), () -> {
            ...
            if(rightClick) {
                PacketHandler.getChannel().sendToServer(new MessageOpenStorage(entity.getId(), TRUNK_STORAGE_KEY));
                Minecraft.getInstance().player.swing(InteractionHand.MAIN_HAND);
            }
        }, entity -> true);
    }
