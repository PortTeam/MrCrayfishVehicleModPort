...
public abstract class PoweredVehicleEntity extends VehicleEntity implements ContainerListener, MenuProvider
{
    ...
    public void fuelVehicle(Player player, InteractionHand hand)
    {
        if(SyncedPlayerData.instance().get(player, ModDataKeys.GAS_PUMP).isPresent())
        {
            ...
            if(stack.isEmpty() || !Config.SERVER.validFuels.get().contains(stack.getFluid().toString()))
                return;
        ...
        if(fluidStack.isEmpty() || !Config.SERVER.validFuels.get().contains(fluidStack.getFluid().toString()))
            return;
    ...
    public void onUpdateVehicle()
    {
        ...
        if(this.requiresEnergy() && controllingPassenger instanceof Player && !((Player) controllingPassenger).isCreative() && this.isEnginePowered())
        {
            ...
            currentFuel -= (float) (this.getEnergyConsumptionPerTick() * Config.SERVER.energyConsumptionFactor.get());
        ...
        if(this.showWheelParticles())
        {
            ...
            if(properties.getWheels() != null)
            {
                ...
                for(int i = 0; i < wheels.size(); i++)
                {
                    ...
                    if(state.getBlock() != Blocks.AIR && state.isSolid())
                    {
                        ...
                        if(this.level.isClientSide())
                        {
                            ...
                            if(this.showTyreSmokeParticles() && SurfaceHelper.getSurfaceTypeForMaterial(state.getBlock()) == SurfaceHelper.SurfaceType.SOLID)
                            {
    ...
    public void onClientUpdate()
    {
        ...
        if(entity instanceof LivingEntity && entity.equals(Minecraft.getInstance().player))
        {
            ...
            if(throttle != this.getThrottle())
            {
                ...
                PacketHandler.getChannel().sendToServer(new MessageThrottle(throttle));
            }
            ...
            if(this.isHandbraking() != handbraking)
            {
                ...
                PacketHandler.getChannel().sendToServer(new MessageHandbrake(handbraking));
            }
            ...
            if(this.hasHorn())
            {
                ...
                this.setHorn(horn);
                PacketHandler.getChannel().sendToServer(new MessageHorn(horn));
            }

            float steeringAngle = VehicleHelper.getSteeringAngle(this);
            this.setSteeringAngle(steeringAngle);
            PacketHandler.getChannel().sendToServer(new MessageTurnAngle(steeringAngle));
        }
    ...
    protected void readAdditionalSaveData(CompoundTag compound)
    {
        ...
        if(compound.contains("StepHeight", CompoundTag.TAG_FLOAT))
        {
            //this.maxUpStep = compound.getFloat("StepHeight");
        }
        if(compound.contains("CurrentFuel", CompoundTag.TAG_FLOAT))
        {
            this.setCurrentEnergy(compound.getFloat("CurrentFuel"));
        }
        if(compound.contains("KeyNeeded", CompoundTag.TAG_BYTE))
        {
    ...
    public boolean causeFallDamage(float distance, float damageMultiplier, DamageSource pSource) {
        ...
        {
            super.causeFallDamage(distance, damageMultiplier,pSource);
        }
    ...
    {
        this.vehicleInventory = new Inventory(((Player) this.getControllingPassenger()));
    ...
    public ItemStack getPickedResult(HitResult target)
    {
        ...
        ResourceLocation entityId = ResourceLocation.tryParse(this.getType().toString());
        if(entityId != null)
        {
    ...
    {
        return new EditVehicleContainer(windowId, this.getVehicleInventory(), this, Player, playerInventory);
    }
