...
public class FluidUtils
{
    ...
    {
        Integer cachedColor = CACHE_FLUID_COLOR.get(fluid);
        if(cachedColor != null)
        {
            return cachedColor;
        }
...
//            }
            CACHE_FLUID_COLOR.put(ResourceLocation.tryParse(fluid.toString()), fluidColor);
            return fluidColor;
        }
    }
    ...
    public static void drawFluidInWorld(FluidTank tank, Level world, BlockPos pos, PoseStack matrixStack, MultiBufferSource renderTypeBuffer, float x, float y, float z, float width, float height, float depth, int light, FluidSides sides)
    {
        ...
        int waterColor = tank.getFluid().getFluid().getAmount(tank.getFluid().getFluid().defaultFluidState());
