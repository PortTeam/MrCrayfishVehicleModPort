...
public class CommonUtils
{
    ...
    {
        if(compound.contains(key, CompoundTag.TAG_COMPOUND)) // Constants.NBT.TAG_COMPOUND changed to CompoundTag.TAG_COMPOUND
        {
    ...
    public static void sendInfoMessage(Player player, String message)
    {
        ...
        {
            player.sendSystemMessage(Component.literal(message)); // displayClientMessage has been replaced by sendSystemMessage
        }
    }
    ...
    public static float pitch(Vector3d vec)
    {
        ...
        {
            // Fixes the absolute of the value being slightly greater than 1.0
            double y = Mth.clamp(vec.normalize().y, -1.0, 1.0);
            // If abs of y is greater than 1.0, returns NaN when calling Math#asin and crashes world
    ...
    {
        return yaw(new Vector3d((Vector3fc) vec));
    }
    ...
    {
        return pitch(new Vector3d((Vector3fc) vec));
    }
    ...
    {
        return motion.normalize().multiply(Mth.clamp(motion.length(), 0F, Config.SERVER.globalSpeedLimit.get()),1,1); // scale() is now mul()
    }
}
